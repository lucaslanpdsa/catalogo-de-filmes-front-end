...
const loadGenres = async () => {
  try {
    const res = await axios.get(`${tmdbApiBaseUrl}genre/movie/list`, {
...
const searchMovies = async (term) => {
  ...
  try {
    const res = await axios.get(`${tmdbApiBaseUrl}search/movie`, {
...
<template>
  ...
  <div class="bg-neutral-900 min-h-screen">
    <div class="pt-4 px-4 flex gap-4 text-sm text-gray-300 select-none justify-center">
      <button
        v-for="tab in tabs"
        :key="tab.key"
    ...
    <div v-if="filteredMovies.length" class="grid grid-cols-2 md:grid-cols-4 gap-6 mt-6 px-2 pb-10">
      <div
      ...
      >
        ...
        <div class="p-4 w-full text-center">
          ...
          <div class="flex justify-center gap-4 text-white text-sm">
